...
export default function CardEducation({ initial_date , finish_date ,higher_education , type , title , institution , location } : IEducation){
 ...
 {initial_date} - {finish_date}
        ...
        <span className="flex items-center font-semibold dark:text-dark-text text-light-text gap-1  dark:bg-dark-background  bg-light-background-transparent shadow-3xl rounded-2xl p-2">  
         <GraduationIcon/>{higher_education}
         </span>
      </div>
      <div className="my-4">
        <span className=" dark:text-dark-text text-light-text gap-2  dark:bg-dark-background  bg-light-background-transparent shadow-3xl rounded-2xl p-1">{type}</span>
      </div>
      <header>
        <h1 className="font-semibold text-xl">{title}</h1>
      </header>
      <div className="flex justify-center flex-col gap-3 mt-2">
        <span  className="flex items-center gap-2">    <UnivercityIcon/>{institution} </span>
        <span  className="flex items-center gap-2">   <LocationIcon/> {location}</span>
      </div>
    </div>
  )
}