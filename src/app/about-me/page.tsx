...
export default function About(){
  ...
  useEffect(() => {
    async function fetchRepos() {
      try {
        const response = await fetch('https://api.github.com/users/Kauan-VX/repos');
...
<span className="flex flex-col items-center justify-center gap-2">
  ...
  <h1 className="font-bold text-4xl text-[#00b294]">{counterOn &&<CountUp scrollSpyDelay={0} start={0} end={timeExperience} duration={5}/>}+</h1>
  <h2 className="text-center font-semibold">Ano(s) de experiência</h2>
</span>
          </li>
          <li className="dark:bg-dark-background-transparent bg-light-background-transparent shadow-3xl rounded-3xl w-40 p-4">
<span className="flex flex-col items-center justify-center gap-2">

  <h1 className="font-bold text-4xl text-[#00b294]">{counterOn &&<CountUp start={0} end={repos.length} duration={5}/>}+</h1>
  <h2 className="text-center font-semibold">Projetos concluídos</h2>
</span>
          </li>
        </ul>
      </div>
